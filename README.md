...
「设置 → 用量统计」记录每次模型调用的 Token 消耗：语音转写、图像识别、文本优化、自动学习四类场景分开统计，按天/按小时查看趋势，按模型查看调用次数和输入输出明细。

- 记录保存在本机，从功能上线后开始累积，不上传
- 服务商不返回用量时按 0 记，调用次数照常统计
- 可一键清空全部记录
...
- 切换更轻量的模型（如 Gemini 3.5 Flash Lite）
- 检查网络延迟
</details>
